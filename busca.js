(function () {
  ...
  function filtrar() {
    ...
    return ALUNOS.filter(function (a) {
      if (matricula && semAcento(a.matricula).indexOf(matricula) === -1) return false;
  ...
  function exibir(lista) {
    ...
    corpoTabela.innerHTML = lista.map(function (a) {
      return '<tr>' +
        '<td>' + escapar(a.matricula) + '</td>' +
        '<td>' + escapar(a.cpf) + '</td>' +
        '<td>' + escapar(a.nome) + '</td>' +
        '<td>' + escapar(a.curso) + '</td>' +
        '<td class="col-acao"><button type="button" class="botao-pdf" data-matricula="' +
          escapar(a.matricula) + '" aria-label="Baixar PDF do histórico de ' + escapar(a.nome) + '">Baixar PDF</button></td>' +
        '</tr>';
    }).join('');
    total.hidden = false;
    total.textContent = lista.length + (lista.length === 1 ? ' aluno encontrado' : ' alunos encontrados');
  }
  ...
  corpoTabela.addEventListener('click', function (evento) {
    ...
    if (!botao) return;
    var aluno = ALUNOS.find(function (a) { return a.matricula === botao.dataset.matricula; });
    if (aluno) gerarPdf(aluno);
  });

  /* ---------- Histórico em PDF (jsPDF + AutoTable) ---------- */
  ...
  function calcularIndices(a) {
    var somaNotaCh = 0, somaCh = 0, chAprovada = 0;
    ...
    return {
      ...
      // Média geral: média ponderada pela carga horária de todas as disciplinas
      mediaGeral: somaCh ? decimal(somaNotaCh / somaCh) : '-',
      // IRA: média das notas ponderada pela carga horária, em todas as disciplinas
      ira: somaCh ? decimal(somaNotaCh / somaCh) : '-'
    };
  }

  function tituloSecao(texto, colunas) {
    return [{ content: texto, colSpan: colunas, styles: { halign: 'center', fillColor: COR_FAIXA, fontStyle: 'bold' } }];
  }

  function rotulo(texto) {
    return { content: texto, styles: { fontStyle: 'bold', fillColor: COR_PAINEL } };
  }

  function gerarPdf(a) {
    if (!window.jspdf || !window.jspdf.jsPDF) {
      alert('Não foi possível carregar a biblioteca de PDF. Verifique a conexão com a internet e recarregue a página.');
      return;
    }
    ...
    var estiloBase = {
      ...
      styles: { font: 'helvetica', fontSize: 8.5, cellPadding: 1.6, lineColor: COR_BORDA, lineWidth: 0.2, textColor: 20 },
      headStyles: { fillColor: COR_CABECALHO, textColor: 20, fontStyle: 'bold' }
    };
    ...
    doc.setFontSize(9);
    doc.text('Instituto Federal do Sudeste de Minas Gerais - Histórico Escolar', margem, 23);
    doc.setDrawColor(COR_AZUL_ESCURO[0], COR_AZUL_ESCURO[1], COR_AZUL_ESCURO[2]);
    ...
    doc.autoTable(Object.assign({}, estiloBase, {
      ...
      body: [
        ...
        [rotulo('Data de nascimento'), formatarData(a.nascimento), rotulo('Telefone'), a.telefone],
        [rotulo('E-mail'), { content: a.email, colSpan: 3 }]
      ],
      columnStyles: { 0: { cellWidth: 40 }, 2: { cellWidth: 40 } }
    }));

    // Dados do curso, com Média geral e IRA lado a lado
    doc.autoTable(Object.assign({}, estiloBase, {
      startY: doc.lastAutoTable.finalY + 5,
      head: [tituloSecao('Dados do Curso', 4)],
      body: [
        [rotulo('Curso'), { content: a.curso, colSpan: 3 }],
        [rotulo('Ano/semestre de ingresso'), a.anoIngresso + '.' + a.semestreIngresso, rotulo('Situação do vínculo'), a.vinculo],
        [rotulo('Carga horária cursada'), indices.chCursada + 'h', rotulo('Carga horária aprovada'), indices.chAprovada + 'h'],
        [rotulo('Média geral'), indices.mediaGeral, rotulo('IRA'), indices.ira]
      ],
      columnStyles: { 0: { cellWidth: 40 }, 2: { cellWidth: 40 } }
    }));
    ...
    a.historico.forEach(function (d) {
      if (d.periodo !== periodoAtual) {
        ...
        linhas.push([{ content: d.numeroPeriodo + 'º período - ' + d.periodo, colSpan: 6,
          styles: { fontStyle: 'bold', fillColor: COR_PAINEL } }]);
      }
      linhas.push([d.periodo, d.disciplina, d.cargaHoraria + 'h', String(d.nota), d.frequencia + '%', d.situacao]);
    });
    ...
    doc.autoTable(Object.assign({}, estiloBase, {
      ...
      head: [
        ...
        ['Período', 'Disciplina', 'Carga horária', 'Nota', 'Frequência', 'Situação']
      ],
      body: linhas,
      showHead: 'everyPage',
      columnStyles: {
      ...
      didParseCell: function (dado) {
        if (dado.section === 'head' && dado.row.index === 1 && dado.column.index >= 2 && dado.column.index <= 4) {
          dado.cell.styles.halign = 'center';
        }
        if (dado.section === 'body' && dado.column.index === 5 && dado.cell.raw === 'Reprovado') {
    ...
    var regra = 'Notas de 0 a 100. Aprovação com nota mínima 60 e frequência mínima de 75%. ' +
    ...
    for (var pagina = 1; pagina <= totalPaginas; pagina++) {
      ...
      doc.setFontSize(7);
      doc.setTextColor(80);
      doc.text(doc.splitTextToSize(regra, largura - margem * 2), margem, altura - 12);
